...
from utils import get_accuracy_bin, get_out_bin
    ...
    for epoch in range(0,epochs):
        ...
        for batch_id, smpl in enumerate(tqdm(train_dataloader)):
            ...
            for m in range(len(tresholds)):
                ...
                acc_ep[m] = accuracy_score(label.cpu(), label_t.cpu())*len(label) + acc_ep[m]
        ...
        for o in range(0,3):
            for batch_id, smpl in enumerate(tqdm(val_dataloader)):
                ...
                for m in range(len(tresholds)):
                    label_t = euclidean_distance<tresholds[m]
                    acc_v[m] = accuracy_score(label.cpu(), label_t.cpu())*len(label) + acc_v[m]
                items = items + len(label)
                batch = batch + 1
...
def trainSiamensB(net, tresholds, epochs, train_dataloader, device, optimizer, criterion, sch, val_dataloader, best_tr=1):
    loss=[] 
    loss_v=[]
    counter=[]
    counter_v=[]
    ...
    for epoch in range(0,epochs):
        ...
        for batch_id, smpl in enumerate(tqdm(train_dataloader)):
            ...
            optimizer.step()
            for m in range(len(tresholds)):
                acc_ep[m] = get_accuracy_bin(label.cpu(), output.cpu(), tresholds[m])*len(label) + acc_ep[m]
            items = items + len(label)
            batch = batch + 1
            out1.append(get_out_bin(output.cpu(),0.75)), out2.append(get_out_bin(output.cpu(),1.0)), out3.append(get_out_bin(output.cpu(),1.25))
            out4.append(get_out_bin(output.cpu(),1.5)), out5.append(get_out_bin(output.cpu(),1.75)), out6.append(get_out_bin(output.cpu(),0.5)), lab.append(label)
        print("Epoch {}\n Current loss {}".format(epoch,loss_contrastive.item()))
        for m in range(len(tresholds)):
            print("Current accuracy {} for {}\n".format(acc_ep[m]/items,tresholds[m]))
            acc[m][epoch] = acc_ep[m]/items
        ...
        for o in range(0,3):
            for batch_id, smpl in enumerate(tqdm(val_dataloader)):
                ...
                img0, img1, label = smpl[0], smpl[1], smpl[2]
                img0, img1, label = img0.to(device), img1.to(device) , label.to(device)
                output1 = net(img0,img1)
                loss_contrastive = criterion(output,label)
                for m in range(len(tresholds)):
                    acc_v[m] = get_accuracy_bin(label.cpu(), output.cpu(), tresholds[m])*len(label) + acc_v[m]
                items = items + len(label)
                batch = batch + 1
        print("Epoch {}\n Current val loss {}".format(epoch,loss_contrastive.item()))
        for m in range(len(tresholds)):
...
def testB(net, tresholds, test_dataloader, device, criterion, best_tr=1):
    net.eval()
    acc_v = np.zeros(len(tresholds))
    items = 0
    batch = 0
